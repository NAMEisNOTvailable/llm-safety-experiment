...
build/
  raw_results/  Ignored scratch outputs from model re-runs
...
| `scripts/merge_human_review_labels.py` | Merge raw runner outputs with human-review labels into final `data/results/` files |
| `scripts/validate_dataset.py` | Validate prompt/result row counts, idx coverage, labels, and adjudication fields |
...
python scripts/model_runner_evaluation.py \
  ...
  --out build/raw_results/glm3_results_English.jsonl \
  --model THUDM/chatglm3-6b

# ChatGLM3-6B on Mandarin prompts
python scripts/model_runner_evaluation.py \
  --in data/prompts/1500_Chinese_prompt.jsonl \
  --out build/raw_results/glm3_results_Chinese.jsonl \
  --model THUDM/chatglm3-6b \
  --zh-mode han_en \
  --no-en-only-hard
...
python scripts/prompt_glm4_English.py \
  --in data/prompts/1500_English_prompt.jsonl \
  --out build/raw_results/glm4_results_English.jsonl \
  --model ZhipuAI/glm-4-9b-chat

# ChatGLM4-9B on Mandarin prompts
python scripts/prompt_glm4_Chinese.py \
  --in data/prompts/1500_Chinese_prompt.jsonl \
  --out build/raw_results/glm4_results_Chinese.jsonl \
  --model ZhipuAI/glm-4-9b-chat \
  --zh-mode han_en \
  --no-en-only-hard
...
python scripts/model_runner_evaluation.py \
  --in data/prompts/1500_English_prompt.jsonl \
  --out build/raw_results/llama2_results_English.jsonl \
  --model meta-llama/Llama-2-13b-chat-hf \
  --offload-dir .cache/hf_offload
...
python scripts/prompt_llama2_Chinese.py \
  --in data/prompts/1500_Chinese_prompt.jsonl \
  --out build/raw_results/llama2_results_Chinese.jsonl \
  --model meta-llama/Llama-2-13b-chat-hf \
  --offload-dir .cache/hf_offload \
  --zh-mode han_en \
  --no-en-only-hard
...
After human review, regenerate the checked-in final result files from raw outputs plus the reviewed classification JSON arrays:
...
The merge script joins rows by `idx`, checks that each `model_answer` matches exactly, moves the raw automatic label into `evaluation_result_gatekeeper_v2`, writes the human label to `evaluation_result`, and stores the review rationale in `adjudication_reason`.
