...
The files are kept in JSONL format so each prompt or response can be inspected, filtered, and scored independently. Result files keep reviewed `model_answer` text, retain the earlier Gatekeeper v2 automatic label in `evaluation_result_gatekeeper_v2`, and store the human-reviewed final label in `evaluation_result`. The review rationale is stored in `adjudication_reason`.
